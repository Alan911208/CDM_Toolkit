/*****************************************************************************
 * Internal Helper: %sas2xls
 * Purpose:  Export a SAS dataset to an Excel sheet via DDE.
 * Parameters:
...
/*****************************************************************************
 * Tool 20: %reviewData
 * Purpose:  Interactive Excel-based data review.  Exports all datasets from
 *           a library to Excel sheets, applies query/crosscheck-based
 *           color coding for visual data inspection.
 *
 * Parameters:
 *   path     = Path to the data library (REQUIRED)
 *   style    = Review style:
 *               FULL   - Export all columns (default)
 *               COMPACT - Exclude STUDYID, DOMAIN, SUBJID, SITEID,
 *                         RANDOM, VISITNUM columns
 *   showFail = Show findings that could not be matched to dataset columns:
 *               YES / NO (default: NO)
 *
 * Usage:
 ...
 *   - Color coding from CROSSCHK / QUERY datasets (if present):
 *       Green (pattern 07):  Data confirmed (finding 001)
 *       Purple (pattern 19): Supported by AE/SAE record (002)
 *       Orange (pattern 24): Supported by lab finding (003)
 *       Blue (pattern 34):   Supported by medical history (004)
 *       Teal (pattern 36):   Supported by operation history (005)
 *       Brown (pattern 38):  Supported by physical exam (006)
 *       Rose (pattern 39):   Supported by treatment/drugs (007)
 *       Dark green (prn 40): Supported by vital sign (008)
 *       Red (pattern 03):    Unknown type (999)
 *       Dark border (1):     Entry comment (021)
 *       Blue border (2):     Answered programme query (022)
 *       Red border (6):      Answered key-in query (023)
 *       Blue dashed (2):     Open programme query (024)
 *       Red dashed (6):      Open key-in query (025)
 *   - A legend on the TOC sheet explaining the color codes.
 *   - If showFail=YES, a separate dataset __DataSetName lists findings
 *     that referenced variables not present in the dataset.
 *
 * Requirements & Limitations:
 *   - REQUIRES Windows operating system.
 *   - REQUIRES Microsoft Excel with DDE enabled.
 *     File > Options > Advanced > General >
 *     Ignore other applications that use Dynamic Data Exchange (DDE) = OFF.
 *   - DDE may not work reliably with 64-bit Office + 64-bit SAS.
 *     Use 32-bit SAS with 32-bit Excel for best results.
 *   - The macro will attempt to launch Excel if not already running.
 *   - Large datasets may cause DDE buffer overflow.
 *   - Not suitable for headless/batch environments without a logged-in
 *     desktop session.
 *
 * Supported Datasets in the Library:
 *   - Any dataset except those in the exclusion list: WVISIT, LABREF,
 *     CMEDDRA, CUSERD, MISPAGE, CWHODD, PAGESTATUS, CROSSCHK,
 *     TRANSLATION, QUERY, FORMATS, PAT (case-insensitive).
 *   - QUERY (optional):  If present, findings are extracted and color-coded
 *     onto the corresponding data sheets.
 *   - CROSSCHK (optional):  If present, findings are also extracted.
 *   - FORMATS (optional):  User-defined formats catalog.
 *
 * Internal helpers:
 *   - %sas2xls:  Exports a SAS dataset to an Excel sheet via DDE.
 *
 * Change History:
 *   10Oct2012   Fangqiu Xiao  Original creation
 *   19May2026   Standalone packaging with bilingual documentation
 *****************************************************************************/
%macro reviewData(path=D:\Temp\tmp, style=FULL, showFail=NO);
    ...
    /* Build crosschk0 from QUERY and/or CROSSCHK datasets if present */
    ...
    %if ^%sysfunc(exist(dReview.query,data)) %then %do;
        proc sql noprint;
            create table Crosschk0 ( studyId Char (20) "Study Identifier",
                SiteId char (10) "Study Site Identifier ",
                usubjid char (10) "Unique Subject Identifier",
                init char (4) "Subject Initials ",
                PageID char (6) "Page Identifier ",
                dataset char(20) "Dataset Name",
                var char(32) "Variable ",
                line num "Line Identifier",
                _Find char (200) "Findings ",
                Finding char (200) "Findings ");
    ...
    %do %while (%scan(TOC &datalist, &kk) ne );
        ...
        %if %scan(TOC &datalist, &kk) ne TOC %then %do;
            data %scan(TOC &datalist, &kk);
                ...
                %if %upcase(&style)=COMPACT %then (drop=studyid domain subjid siteid random visitnum);;
            run;
        ...
        %if &go_on>0 %then %do;
            ...
            %if %upcase(&showFail) eq YES %then %do;
                ...
                quit;
                %if &myOBS0>0 %then %put WARNING: There are %cmpres(&myOBS0) items not marked in %scan(TOC &datalist, &kk);
                %else %do;
                    proc sql noprint;
