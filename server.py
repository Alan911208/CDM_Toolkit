...
async def api_file_tree(path: str = ""):
    """Return directory tree as both TXT and HTML-ready JSON."""
    if not path:
        import string
        drives = []
    ...
    try:
        for entry in sorted(os.scandir(path), key=lambda e: (not e.is_dir(), e.name.lower())):
            info = {"name": entry.name, "path": entry.path, "is_dir": entry.is_dir()}
            if not entry.is_dir():
                try: info["size"] = entry.stat().st_size
                except OSError: info["size"] = 0
            items.append(info)
...
@app.post("/api/file-tree-txt")
async def api_file_tree_txt(path: str = Form("")):
    """Generate full recursive tree as downloadable TXT."""
