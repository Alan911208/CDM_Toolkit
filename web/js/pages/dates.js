...
export default function render() {
  return `<div class="page-section"><h2>📅 日期计算</h2><p class="section-subtitle">对日期列批量加减天数</p></div>
...
export async function init() {
  ...
  btn.addEventListener('click', async () => {
    ...
    progress.show(); progress.setProgress(-1, '处理中...'); btn.disabled = true;
    try { const res = await api('/api/date-calc', fd); const blob = await res.blob(); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'dates_result.xlsx'; a.click(); progress.setProgress(100, '完成！'); showToast('日期计算完成'); }
