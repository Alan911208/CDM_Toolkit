...
export default function render() {
  return `<div class="page-section"><h2>✂️ 工作表拆分</h2><p class="section-subtitle">按指定列的值将工作表拆分为多个 sheet</p></div>
...
export async function init() {
  ...
  btn.addEventListener('click', async () => {
    ...
    progress.show(); progress.setProgress(-1, '正在拆分...'); btn.disabled = true;
    try { const res = await api('/api/split', fd); const blob = await res.blob(); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'split.xlsx'; a.click(); URL.revokeObjectURL(url); progress.setProgress(100, '拆分完成！'); showToast('工作表拆分成功'); }
