...
export default function render() {
  return `<div class="page-section"><h2>⚖️ 实验室单位转换</h2><p class="section-subtitle">将实验室检验结果统一转换为标准单位</p></div>
...
export async function init() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try { const res = await api('/api/unit-convert', fd); const blob = await res.blob(); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'units_result.xlsx'; a.click(); progress.setProgress(100, '完成！'); showToast('单位转换完成'); }
